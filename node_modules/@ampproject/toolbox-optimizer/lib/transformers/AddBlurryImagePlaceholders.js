...
class AddBlurryImagePlaceholders {
  constructor(config) {
    ...
    this.maxBlurredPlaceholders_ =
      config.maxBlurredPlaceholders || MAX_BLURRED_PLACEHOLDERS;
    // used for resolving image files
    this.pathResolver_ = new PathResolver(config.imageBasePath);

    // setup caching
    const maxCacheSize =
      config.blurredPlaceholdersCacheSize || DEFAULT_CACHED_PLACEHOLDERS;
    ...
    if (maxCacheSize === CACHE_ALL_PLACEHOLDERS) {
      ...
      this.log_.debug(
        "using LRU cache for regularily used placeholders",
        maxCacheSize
      );
  ...
  async getDataURI_(src) {
    ...
    const imgDimension = this.getBitmapDimensions_(
      image.bitmap.width,
      image.bitmap.height
    );
    image.resize(
