...
function buildPrivateNamesNodes(privateNamesMap, loose, state) {
  ...
  for (const [name, value] of privateNamesMap) {
    ...
    if (loose) {
      ...
      if (isAccessor) {
        initNodes.push(
          _core().template.statement.ast`var ${id} = new WeakMap();`
        );
      } else {
        initNodes.push(
          _core().template.statement.ast`var ${id} = new WeakSet();`
        );
      }
    } else if (!isStatic) {
      initNodes.push(
...
function buildPrivateMethodDeclaration(prop, privateNamesMap, loose = false) {
  ...
  const methodValue = _core().types.functionExpression(
    methodId,
    params,
    body,
    generator,
  ...
  if (isGetter) {
    ...
    return _core().types.variableDeclaration("var", [
      _core().types.variableDeclarator(getId, methodValue)
    ]);
  }
  ...
  if (isSetter) {
    ...
    return _core().types.variableDeclaration("var", [
      _core().types.variableDeclarator(setId, methodValue)
    ]);
  }
  ...
  if (isStatic && !loose) {
    return _core().types.variableDeclaration("var", [
      _core().types.variableDeclarator(
        id,
        _core().types.functionExpression(id, params, body, generator, async)
      )
    ]);
  }

  return _core().types.variableDeclaration("var", [
    _core().types.variableDeclarator(methodId, methodValue)
  ]);
}
