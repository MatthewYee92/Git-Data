  ...
  enter(path, self) {
    ...
    if (path.isForStatement()) {
      ...
      path.replaceWithMultiple(
        self.pushDeclar(node).map(expr => _core.types.expressionStatement(expr))
      );
...
class BlockScoping {
      ...
      for (const violation of binding.constantViolations) {
        ...
        if (violation.isAssignmentExpression()) {
          ...
          violation.replaceWith(
            _core.types.sequenceExpression([throwNode, violation.node])
          );
  ...
  wrapClosure() {
    ...
    const args = (0, _values.default)(outsideRefs).map(id =>
  ...
  buildHas(ret) {
    ...
    if (has.hasBreakContinue) {
      for (const key of Object.keys(has.map)) {
        cases.push(
          _core.types.switchCase(_core.types.stringLiteral(key), [has.map[key]])
        );
      }
      ...
      if (cases.length === 1) {
        ...
        body.push(
          _core.types.switchStatement(_core.types.identifier(ret), cases)
        );
      }
    } else {
      if (has.hasReturn) {
