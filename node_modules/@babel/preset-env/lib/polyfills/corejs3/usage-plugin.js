...
function _getModulesListForTargetVersion() {
  const data = _interopRequireDefault(
    require("core-js-compat/get-modules-list-for-target-version")
  );
...
const corejs3PolyfillsWithoutProposals = Object.keys(_data().default)
...
const corejs3PolyfillsWithShippedProposals = _shippedProposals.default.reduce(
  (memo, key) => {
    memo[key] = _data().default[key];
    return memo;
  },
...
function _default(
  ...
  const polyfills = (0, _filterItems.default)(
    proposals
      ? _data().default
      : shippedProposals
  ...
  const available = new Set(
    (0, _getModulesListForTargetVersion().default)(corejs.version)
  );
  ...
  const addAndRemovePolyfillImports = {
    ...
    Program(path) {
      path.get("body").forEach(bodyPath => {
        if (
          (0, _utils.isPolyfillSource)((0, _utils.getRequireSource)(bodyPath))
        ) {
          console.warn(NO_DIRECT_POLYFILL_IMPORT);
          bodyPath.remove();
        }
  ...
  return {
    ...
    pre() {
      ...
      this.addPropertyDependencies = function(source = {}, key) {
        ...
        if (_builtInDefinitions.PossibleGlobalObjects.has(builtIn)) {
          ...
          (0, _utils.has)(_builtInDefinitions.StaticProperties, builtIn)
        ) {
          const BuiltInProperties =
            _builtInDefinitions.StaticProperties[builtIn];
        ...
        if (!(0, _utils.has)(_builtInDefinitions.InstanceProperties, key))
          return;
        let InstancePropertyDependencies =
          _builtInDefinitions.InstanceProperties[key];

        if (instanceType) {
          InstancePropertyDependencies = InstancePropertyDependencies.filter(
            m =>
              m.includes(instanceType) ||
    ...
    post({ path }) {
      const filtered = (0, _utils.intersection)(
        polyfills,
        this.polyfillsSet,
        available
      );
